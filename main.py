...
def evaluate(data_source, batch_size=10):
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            total_loss += len(data) * criterion(output, targets) #criterion(model.decoder.weight, model.decoder.bias, output, targets.view(-1)).data
...
def train(epoch=0):
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        output, hidden, mems, attn_outs, _ = model(data, hidden, return_h=True, mems=mems)
        raw_loss = criterion(output, targets) #criterion(model.decoder.weight, model.decoder.bias, output, targets.view(-1))
